...
api.interceptors.response.use(
  (response) => {
    // Show success toast for successful operations (POST, PUT, DELETE, PATCH)
    if (globalToastHandler && ['post', 'put', 'delete', 'patch'].includes(response.config.method)) {
      const successMessage = response.data?.message || 'Operation completed successfully';
      globalToastHandler.showSuccess('Success', successMessage);
    }
    return response;
  },
  (error) => {
    ...
    if (globalToastHandler) {
      globalToastHandler.handleApiError(error);
    }
...
// Enhanced API functions with automatic toast handling
const createApiWithToast = (baseApi) => {
  const enhancedApi = {};
  
  Object.keys(baseApi).forEach(key => {
    const originalFunction = baseApi[key];
...
export const vmServiceAuth = createApiWithToast({
...
export const vmServiceOverview = createApiWithToast({
...
export const bookingsAPI = createApiWithToast({
  // Create new booking
...
export const customersAPI = createApiWithToast({
...
export const vehiclesAPI = createApiWithToast({
...
export const servicesAPI = createApiWithToast({
...
export const inventoryAPI = createApiWithToast({
...
export const paymentsAPI = createApiWithToast({
  // Get all payments
...
export const usersAPI = createApiWithToast({
  // Get all users
  getUsers: async () => {
...
// User Management API
export const userManagementAPI = createApiWithToast({
