...
const InventoryManagementNew = () => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      // Check if response has items property (direct API response)
      if (response.items) {
  ...
  const handleInputChange = (formType, field, value) => {
    ...
    } else if (formType === 'stockAdjustment') {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          ...
          <Button
            onClick={() => (activeTab === 'items' ? setShowItemForm(true) : setShowSupplierForm(true))}
            className="bg-orange-600 hover:bg-orange-700 text-white"
          >
            <Plus className="h-4 w-4 mr-2" />
            Add {activeTab === 'items' ? 'Item' : 'Supplier'}
          </Button>
        </div>
      </div>
...
const SuppliersTab = ({ suppliers, handleEditSupplier, handleDeleteSupplier }) => {
  return (
    <div className="space-y-6">
      <Card className="border-0 shadow-sm">
        <CardHeader>
